...
miniLock.crypto.createHeader = function(
	miniLockIDs,
	myMiniLockID,
	mySecretKey,
	fileKey,
...
miniLock.crypto.decryptHeader = function(header, mySecretKey, myMiniLockID) {
	if (
...
miniLock.crypto.decryptFile = function(
	...
	miniLock.file.read(file, 8, 12, function(headerLength) {
		...
		miniLock.file.read(file, 12, headerLength + 12, function(header) {
			...
			var actualDecryptInfo = miniLock.crypto.decryptHeader(
				header, mySecretKey, myMiniLockID
			)
			if (!actualDecryptInfo) {
	...
	miniLock.file.read(
		...
		function(chunk) {
			...
			if (isLast) {
				if (
					!nacl.verify(
						new Uint8Array(hashObject.digest()),
